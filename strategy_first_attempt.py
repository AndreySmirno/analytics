...
x = reset_i(np.log(sber_d['close'] / sber_d['close'].shift(1)).dropna())
# x.plot()
...
# Собрать тест на нормальность и стационарность вместе, как тест на валидность, для оценки GARCH модели и
# кормить модел ьне всеми данными, а окном, после валидации и делать 1 прогнозов вперёд, потом проводить оценку валидности модели
# ёпта!
# Важно, что ещё есть нейронка, которая улушает значения для модели, вот её надо не просто переобуитвать, а по
# хорошему дополнять модель данными, чтобы она набирала силу со временем и точность прогноза росла.

stationarity_res = stationary_test(x[:50000])
...
# def garch_implementation(data, data_feeder_eval=int):
#     garch_res = []
#     predictions_one_step_ahead = []
#     for i in range(data_feeder_eval, len(data)-1):
#        seq = data[(i-data_feeder_eval):i]
#        forward = data[(i+1):(i+2)]
#        if stationary_test(seq) == 1:
#            # print('Yeah, thats works')
...
#            garch_res.append(garch_volatility)
#            ff = garch.forecast(horizon=1)
#            predicted = ff.mean['h.1'].iloc[-1]
#            forecast_validation = pd.DataFrame({'forecast': predicted, 'actual':forward}, index=[0])
#            predictions_one_step_ahead.append(forecast_validation)
#        else:
#            print('No, fucking way')
#     return garch_res, predictions_one_step_ahead


arch_m = arch_model(x[:30000], vol='GARCH', p=1, q=1, dist='Normal')
garch = arch_m.fit(disp='off')
garch_volatility = np.sqrt(garch.params['omega'] + garch.params['alpha[1]'] * garch.resid ** 2 +
                           garch.conditional_volatility ** 2 * garch.params['beta[1]'])
